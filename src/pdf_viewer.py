...
class PDFViewer(tk.Frame):
    """Main Tk viewer orchestrating OCR, translation, exporting, and KG generation."""
    ...
    def _build_ui(self) -> None:
        ...
        # Paned window (classic Tk variant for broader compatibility)
    ...
    def _set_initial_sash_position(self) -> None:
        try:
            ...
            self.paned_window.sash_place(0, int(width * 0.55), 0)
        except Exception:
            pass
    ...
    def on_toolbar_button_clicked(self, _event=None) -> None:
        ...
        match mode:
            case PdfViewerToolbarItem.SafeArea:
                self._set_status("安全区域模式：拖动红色矩形可限定正文范围。")
            case PdfViewerToolbarItem.Body:
                self._set_status("正文模式：点击元素即可切换是否计入正文。")
            case PdfViewerToolbarItem.Order:
                self._set_status("排序模式：先选锚点，再选其它段落移动位置。按 Esc 取消。")
    ...
    def on_element_left_clicked_by_canvas(self, _event=None) -> None:
        ...
        if mode == PdfViewerToolbarItem.Body:
            self.pdf.toggle_body(key)
        elif mode == PdfViewerToolbarItem.Visibility:
            self.pdf.toggle_visibility(key)
        elif mode == PdfViewerToolbarItem.Order:
            pivot = self.canvas.get_pivot()
            if pivot is None:
                self.canvas.set_pivot(key)
                self._set_status("已设置锚点，再次点击其它段落可移动位置。")
                return
            moved = self.pdf.move_element(pivot, key, self.canvas.get_current_page(), "after")
            if moved:
                self.canvas.set_pivot(key)
                self._set_status("段落顺序已更新。")
            else:
                self._set_status("无法调整顺序，请重新选择。")
                return
        else:
            return
    ...
    def on_element_right_clicked_by_canvas(self, _event=None) -> None:
        ...
        mode = self.toolbar.get_current_selection()
        if mode == PdfViewerToolbarItem.Order:
            pivot = self.canvas.get_pivot()
            if pivot is not None and pivot != key:
                moved = self.pdf.move_element(pivot, key, self.canvas.get_current_page(), "before")
                if moved:
                    if self._persist_context():
                        self.canvas.redraw()
                        self._refresh_text_widget()
                    self._set_status("段落已移动到锚点之前。")
                return
    ...
    def _ensure_font_for_translations(self) -> None:
        ...
        for candidate in [
            ...
            if candidate in available:
                try:
                    current = tkfont.Font(font=self.text_widget["font"])
    ...
    def _persist_context(self) -> bool:
        """Persist PDF context to disk, showing an error on failure."""
        try:
            self.pdf.save()
            return True
